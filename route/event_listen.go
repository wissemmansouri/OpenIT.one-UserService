...
import (
	...
	message_bus "github.com/wissemmansouri/OpenIT.one-UserService/codegen/message_bus"
...
func EventListen() {
	for i := 0; i < 1000; i++ {
		...
		for {
			...
			// logger.Info("info", zap.Any("Write information 1", model))
			// output, err := json.MarshalIndent(event, "", "  ")
			// if err != nil {
			// 	logger.Error("err", zap.Any("err", err.Error()))
			// }
			// logger.Info("info", zap.Any("Write information", string(output)))
		}
	}
